from wtforms import Form, TextField, TextAreaField, SelectField, SelectMultipleField, DateTimeField, FileField, widgets, validators, IntegerField, DateField
...
class EventForm(Form):
  ...
  external_url = TextField('External URL (in an iframe or linked to)', [validators.Optional(),validators.URL()])
  external_width = IntegerField('Width of the iframe (if embedded)', [validators.Optional(),validators.NumberRange(min=0,max=700)])
  external_height = IntegerField('Height of the iframe (if embedded)', [validators.Optional(),validators.NumberRange(min=0,max=500)])
